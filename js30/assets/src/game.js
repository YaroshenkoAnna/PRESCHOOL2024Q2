...
class Cell {
  ...
  open() {
    ...
    if (this.isBomb) {
      ...
      click.play();
      isAllValueOpen();
      defineNeighbors(this.coordinates.j, this.coordinates.i).forEach(function (
        elem
      ) {
        elem.open();
      });
...
function countBombs(j, i) {
  ...
  const neighbors = defineNeighbors(j, i);
...
function showModal(event) {
  ...
  if (event == modalVictory) {
    ...
    winTime.textContent = `Victory! Your Time is ${parseInt(
      time.textContent,
      10
    )}s.`;
...
function closeModal(event) {
  if (
    ...
    (event.target.classList.contains("modal") &&
      !event.target.classList.contains("game__victory"))
  ) {
...
function makeActive(event) {
  ...
  if (
    eClass.contains("modal__ranking-option") &&
    !eClass.contains("modal__ranking-option_active")
  ) {
    document
      .querySelector(".modal__ranking-option_active")
      .classList.remove("modal__ranking-option_active");
...
function getAndSetResults() {
  ...
  if (localStorage.getItem(`leaderboardKey4993${difficult}`)) {
    leaderboard = JSON.parse(
      localStorage.getItem(`leaderboardKey4993${difficult}`)
    );
...
function createRanking(showedRanking) {
  let rankingList = document.getElementById("rankingList");
  rankingList.textContent = "";
  let leaderboard = [];
  if (localStorage.getItem(`leaderboardKey4993${showedRanking}`)) {
    leaderboard = JSON.parse(
      localStorage.getItem(`leaderboardKey4993${showedRanking}`)
    );
  }
  ...
  rankingTime.textContent = "Time, s";
